function output = heatmaps(NET_FILE, ROU_FILE, TRACI_PORT)
    ...
    for i = 1:howmanyfiles
        ...
        travel_time_results = table2array(results(:,18));
        pause(5)
        timer = timer + 5;
        ...
        if repository(i,4) == 1
            ...
            co2gkm_name = [first_number, period, second_number, period, scenario, '_co2gkm_heatmap_Boavista.png'];
        elseif repository(i,4) == 2
            HOUR = num2str(repository(i,2),2);
            first_number = HOUR(1);
            second_number = HOUR(2);
            co2gkm_name = [first_number, period, second_number, period, scenario, '_co2gkm_heatmap_SaoBento.png'];
        else
            HOUR = num2str(repository(i,2),2);
            first_number = HOUR(1);
            second_number = HOUR(2);
            co2gkm_name = [first_number, period, second_number, period, scenario, '_co2gkm_heatmap_A3.png'];
        end
        % create .png image
        ...
        if repository(i,4) == 1
            ...
            first_number = HOUR(1);
            second_number = HOUR(2);
            emissions_indicator_name = [first_number, period, second_number, period, scenario, '_emissions_indicator_heatmap_Boavista.png'];
        elseif repository(i,4) == 2
            HOUR = num2str(repository(i,2),2);
            first_number = HOUR(1);
            second_number = HOUR(2);
            emissions_indicator_name = [first_number, period, second_number, period, scenario, '_emissions_indicator_heatmap_SaoBento.png'];
        else
            HOUR = num2str(repository(i,2),2);
            first_number = HOUR(1);
            second_number = HOUR(2);
            emissions_indicator_name = [first_number, period, second_number, period, scenario, '_emissions_indicator_heatmap_A3.png'];
        end
        % create .png image
        ...
        if repository(i,4) == 1
            ...
            first_number = HOUR(1);
            second_number = HOUR(2);
            eco_indicator_name = [first_number, period, second_number, period, scenario, '_eco_indicator_heatmap_Boavista.png'];
        elseif repository(i,4) == 2
            HOUR = num2str(repository(i,2),2);
            first_number = HOUR(1);
            second_number = HOUR(2);
            eco_indicator_name = [first_number, period, second_number, period, scenario, '_eco_indicator_heatmap_SaoBento.png'];
        else
            HOUR = num2str(repository(i,2),2);
            first_number = HOUR(1);
            second_number = HOUR(2);
            eco_indicator_name = [first_number, period, second_number, period, scenario, '_eco_indicator_heatmap_A3.png'];
        end
        % create .png image
        ...
        % -------------------------------------------------------------------------
        % set up travel_time heatmap
        travel_time_percentiles = [6 8 12];
        for u = 1:length(lanes_edgeID)
            edge = strtrim(lanes_edgeID(u,:));
            for j = 1:length(edges_results)
                edge_results = strtrim(edges_results(j,:));
                compare = strcmp(edge, edge_results);
                if compare == 1
                    if travel_time_results(j,1) < travel_time_percentiles(1,1)
                        traci.edge.setMaxSpeed(edge,1.50);
                    elseif travel_time_results(j,1) > travel_time_percentiles(1,1) && travel_time_results(j,1) < travel_time_percentiles(1,2)
                        traci.edge.setMaxSpeed(edge,2.10);
                    elseif travel_time_results(j,1) > travel_time_percentiles(1,2) && travel_time_results(j,1) < travel_time_percentiles(1,3)
                        traci.edge.setMaxSpeed(edge,6.40);
                    else
                        traci.edge.setMaxSpeed(edge,8.90);
                    end
        ...
        travel_time_SC = imread('travel_time_heatmap_draft.png');
        original_dim = size(travel_time_SC);
        
        % crop top
        lines_to_crop_top = round((0.23*original_dim(1,1))/2);
        travel_time_SC(1:lines_to_crop_top,:,:) = [];
        
        new_dim = size(travel_time_SC);
        % crop bottom
        lines_to_crop_bottom = round((0.25*new_dim(1,1))/2);
        travel_time_SC(new_dim(1,1)-lines_to_crop_bottom:end,:,:) = [];
        
        % crop sides
        % left
        cols_to_crop_left = round((0.21*new_dim(1,2))/2);
        travel_time_SC(:,1:cols_to_crop_left,:) = [];
        
        new_dim = size(travel_time_SC);
        % right
        cols_to_crop_right = round((0.21*new_dim(1,2))/2);
        travel_time_SC(:,new_dim(1,2)-cols_to_crop_right:end,:) = [];
        
        final_dim = size(travel_time_SC);
        % read scale fig
        scale = imread('scale_500m.png');
        scale_dim = size(scale);
        ...
        % read label
        label_travel_time = imread('travel_time_label.png');
        label_dim = size(label_travel_time);
        
        % prepare final image
        travel_time_SC(final_dim(1,1) + 1 - scale_dim(1,1):end,1:scale_dim(1,2),:) = scale;
        travel_time_SC(1:compass_dim(1,1),1:compass_dim(1,2),:) = compass;
        travel_time_SC(1:label_dim(1,1),final_dim(1,2) + 1 - label_dim(1,2):end,:) = label_travel_time;
        ...
        if repository(i,4) == 1
            ...
            first_number = HOUR(1);
            second_number = HOUR(2);
            travel_time_name = [first_number, period, second_number, period, scenario, '_travel_time_heatmap_Boavista.png'];
        elseif repository(i,4) == 2
            HOUR = num2str(repository(i,2),2);
            first_number = HOUR(1);
            second_number = HOUR(2);
            travel_time_name = [first_number, period, second_number, period, scenario, '_travel_time_heatmap_SaoBento.png'];
        else
            HOUR = num2str(repository(i,2),2);
            first_number = HOUR(1);
            second_number = HOUR(2);
            travel_time_name = [first_number, period, second_number, period, scenario, '_travel_time_heatmap_A3.png'];
        end
        % create .png image
        imwrite(travel_time_SC,travel_time_name)
        % -------------------------------------------------------------------------
        % detect edges to return the baseline heatmap per route evaluation
        %     f = fopen('Trips8AM9AM.rou.txt');
        for t = 1:howmany_rou_files
            if rou_repository(t,5) == 1 && rou_repository(t,2) == repository(i,2) && rou_repository(t,3) == repository(i,3) && rou_repository(t,6) == repository(i,6)
                ...
                if repository(i,4) == 1
                    ...
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    co2gkm_name = [first_number, period, second_number, period,'_co2gkm_heatmap_Boavista_baseline_routes.png'];
                elseif repository(i,4) == 2
                    HOUR = num2str(repository(i,2),2);
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    co2gkm_name = [first_number, period, second_number, period, '_co2gkm_heatmap_SaoBento_baseline_routes.png'];
                else
                    HOUR = num2str(repository(i,2),2);
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    co2gkm_name = [first_number, period, second_number, period, '_co2gkm_heatmap_A3_baseline_routes.png'];
                end
                % create .png image
                imwrite(CO2_SC,co2gkm_name)
                % -------------------------------------------------------------------------
                ...
                if repository(i,4) == 1
                    ...
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    emissions_indicator_name = [first_number, period, second_number, period, '_emissions_indicator_heatmap_Boavista_baseline_routes.png'];
                elseif repository(i,4) == 2
                    HOUR = num2str(repository(i,2),2);
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    emissions_indicator_name = [first_number, period, second_number, period, '_emissions_indicator_heatmap_SaoBento_baseline_routes.png'];
                else
                    HOUR = num2str(repository(i,2),2);
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    emissions_indicator_name = [first_number, period, second_number, period, '_emissions_indicator_heatmap_A3_baseline_routes.png'];
                end
                % create .png image
                imwrite(Emissions_Ind_SC,emissions_indicator_name)
                % -------------------------------------------------------------------------
                ...
                if repository(i,4) == 1
                    ...
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    eco_indicator_name = [first_number, period, second_number, period, '_eco_indicator_heatmap_Boavista_baseline_routes.png'];
                elseif repository(i,4) == 2
                    HOUR = num2str(repository(i,2),2);
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    eco_indicator_name = [first_number, period, second_number, period, '_eco_indicator_heatmap_SaoBento_baseline_routes.png'];
                else
                    HOUR = num2str(repository(i,2),2);
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    eco_indicator_name = [first_number, period, second_number, period, '_eco_indicator_heatmap_A3_baseline_routes.png'];
                end
                % create .png image
                imwrite(eco_indicator_SC,eco_indicator_name)
                ...
                % -------------------------------------------------------------------------
                % set up travel time heatmap per route (baseline)
                travel_time_percentiles = [6 8 12];
                for u = 1:length(lanes_edgeID)
                    edge = strtrim(lanes_edgeID(u,:));
                    for j = 1:length(edges_results)
                ...
                travel_time_SC = imread('travel_time_heatmap_baseline_routes_draft.png');
                original_dim = size(travel_time_SC);
                ...
                if repository(i,4) == 1
                    ...
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    travel_time_name = [first_number, period, second_number, period, '_travel_time_heatmap_Boavista_baseline_routes.png'];
                elseif repository(i,4) == 2
                    HOUR = num2str(repository(i,2),2);
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    travel_time_name = [first_number, period, second_number, period, '_travel_time_heatmap_SaoBento_baseline_routes.png'];
                else
                    HOUR = num2str(repository(i,2),2);
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    travel_time_name = [first_number, period, second_number, period, '_travel_time_heatmap_A3_baseline_routes.png'];
                end
                % create .png image
                imwrite(travel_time_SC,travel_time_name)
                % -------------------------------------------------------------------------
            elseif rou_repository(t,5) == 2 && rou_repository(t,2) == repository(i,2) && rou_repository(t,3) == repository(i,3) && rou_repository(t,6) == repository(i,6)
                % -------------------------------------------------------------------------
                % Optimized scenario
                ...
                solution_value = num2str(repository(i,6));
                scenario = ['_optimal_routes', '_solution', solution_value];
                ...
                % -------------------------------------------------------------------------
                % set up travel time heatmap per route (optimal)
                travel_time_percentiles = [6 8 12];
                for u = 1:length(lanes_edgeID)
                    edge = strtrim(lanes_edgeID(u,:));
                    for j = 1:length(edges_results)
                ...
                travel_time_SC = imread('travel_time_heatmap_optimal_routes_draft.png');
                original_dim = size(travel_time_SC);
                ...
                if repository(i,4) == 1
                    ...
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    travel_time_name = [first_number, period, second_number, period,'_travel_time_heatmap_Boavista', scenario, '.png'];
                elseif repository(i,4) == 2
                    HOUR = num2str(repository(i,2),2);
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    travel_time_name = [first_number, period, second_number, period, '_travel_time_heatmap_SaoBento', scenario, '.png'];
                else
                    HOUR = num2str(repository(i,2),2);
                    first_number = HOUR(1);
                    second_number = HOUR(2);
                    travel_time_name = [first_number, period, second_number, period, '_travel_time_heatmap_A3', scenario, '.png'];
                end
                % create .png image
                imwrite(travel_time_SC,travel_time_name)
            end
